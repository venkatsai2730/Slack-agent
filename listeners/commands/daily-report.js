...
module.exports = (app) => {
  app.command('/daily-report', async ({ command, ack, respond, client, logger }) => {
    await ack();
    await respond({ response_type: 'ephemeral', text: '📝 Writing today\'s impact report...' });

    try {
      const report = await buildDailyReport();
      const message = { blocks: reportBlocks(report), text: `Daily Impact Report — ${report.stats.date}` };

      const target = process.env.IMPACT_CHANNEL;
      try {
        await client.chat.postMessage({ channel: target || command.channel_id, ...message });
      } catch (err) {
        if (!target) throw err;
        // Bot not in IMPACT_CHANNEL (or bad name) — post here instead so the demo never stalls.
